...
class AnaliticView extends Model
{
    protected $table = 'analitic_views';
    protected $fillable = ['user_id', 'time', 'views', 'course', 'type'];

    public static function addAnalitic($user, $id, $type){
    	Self::create([
    		'user_id' => $user,
    		'time' => time(),
    		'views' => 1,
    ...
    public static function getCountView(){
        ...
        $result = Self::where('time', '>', $stat_min)->where('user_id', Auth::user()->id);
        $result = $result->where('time', '<', $stat_max);
    ...
    public static function getCountViewInfo($id, $type, $r){
        if(isset($r->info)){
            $x2_month = 2629743 * 2;
            $x2_day = 86400 * 2;
            $x2_week = 604800 * 2;
